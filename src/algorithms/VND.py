...
def swap_kt(n_vehicles, n_days, vehicle_capacity, data, sorted_data, distance_matrix, closeness_matrix, current_solution, value_SEED, neighbour, clients_NO_max_frequ):
    ...
    # final_comb.print()
    # new_solution.print()
    # print("OBJ_tot_dist", new_solution.OBJ_tot_dist)
...
def def_initial_comb(neighbour, n_vehicles, n_days, data, sorted_data, current_solution, value_SEED, clients_NO_max_frequ):
    ...
    if neighbour == 1:
        ...
        # print("initial_comb_1.day", initial_comb_1.day)
        # all_days = [i for i in range(n_days)]
        # print("all_days", all_days)
        # possible_days = [i for i in range(n_days) if i != initial_comb_1.day]
        # print("possible_days", possible_days)
        # possible_days_wrong = [i for i in range(n_days-1) if i != initial_comb_1.day]
        # print("possible_days_wrong", possible_days_wrong)
        ...
        (real_client_index_2, error_index) = pick_client(n_days, data, current_solution, vehicle_i, day_i2, clients_NO_max_frequ, initial_comb_1.client)
    ...
    if neighbour == 3:
        ...
        # print("initial_comb_1.vehicle", initial_comb_1.vehicle)
        # all_vehicles = [i for i in range(n_vehicles)]
        # print("all_vehicles", all_vehicles)
        # possible_vehicles = [i for i in range(n_vehicles) if i != initial_comb_1.vehicle]
        # print("possible_vehicles", possible_vehicles)
        ...
        (real_client_index_2, error_index) = pick_client(n_days, data, current_solution, vehicle_i2, day_i2, clients_same_freq, 0)
        if error_index == False:
            initial_comb = 0
    ...
    if neighbour == 4:
        ...
        day_i2 = random.randint(0, n_days-1)
        (real_client_index_2, error_index) = pick_client(n_days, data, current_solution, vehicle_i2, day_i2, 0, 0)
        schedule_i2 = define_initial_schedule(n_days, current_solution, vehicle_i2, real_client_index_2)
...
def def_final_comb(neighbour, n_vehicles, n_days, vehicle_capacity, data, current_solution, initial_comb, value_SEED):
    ...
    if neighbour == 4:
        ...
        # print("new_transp_demand_matrix", new_transp_demand_matrix)
            # print("hello")
        ...
        # print("available_days_1", available_days_1)
        # print("available_days_2", available_days_2)
        ...
        # Final combination:
        final_comb_1 = classes.Move_combination('final_1', initial_comb.comb_2.vehicle, available_days_1, schedule_f1, initial_comb.comb_1.client)
...
def move_operation(n_vehicles, n_days, data, distance_matrix, closeness_matrix, current_solution, initial_comb, final_comb):
    ...
    '''
    # NEW !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    (new_assigned_ordered_matrix, new_transp_demand_matrix, new_route_dist_matrix) = remove_add_client_NEW(
        n_vehicles, n_days, data, distance_matrix, closeness_matrix, initial_comb, final_comb, new_route_dist_matrix, new_assigned_ordered_matrix, new_transp_demand_matrix)
    print("\nnew_assigned_ordered_matrix\n", new_assigned_ordered_matrix)
    '''
...
def remove_add_client(n_days, data, initial_comb, final_comb, new_assigned_ordered_matrix, new_transp_demand_matrix):
    ...
    for day in range(n_days):
        ...
        (new_assigned_ordered_matrix, new_transp_demand_matrix) = remove_client(n_days, data, initial_comb, final_comb, new_assigned_ordered_matrix, new_transp_demand_matrix, day)      
        ...
        (new_assigned_ordered_matrix, new_transp_demand_matrix) = add_client(n_days, data, initial_comb, final_comb, new_assigned_ordered_matrix, new_transp_demand_matrix, day)      
...
def find_best_route_OLD(n_vehicles, n_days, distance_matrix, closeness_matrix, initial_comb, final_comb, new_route_dist_matrix, new_assigned_ordered_matrix):
...
def remove_add_client_NEW(n_vehicles, n_days, data, distance_matrix, closeness_matrix, initial_comb, final_comb, new_route_dist_matrix, new_assigned_ordered_matrix, new_transp_demand_matrix):
    ...
    for day in range(n_days):    
        ...
        if initial_comb.schedule[day] == 1:
            ...
            new_transp_demand_matrix[initial_comb.vehicle, day] -= data[initial_comb.client, conf.DEMAND_INDEX]
            new_route_dist_matrix[initial_comb.vehicle, day] = sum(distance_matrix[new_client_list[i], new_client_list[i + 1]] for i in range(len(new_client_list) - 1))
        ...
        if final_comb.schedule[day] == 1:
            ...
            new_transp_demand_matrix[final_comb.vehicle, day] += data[final_comb.client, conf.DEMAND_INDEX]
            new_route_dist_matrix[final_comb.vehicle, day] = sum(distance_matrix[new_client_list[i], new_client_list[i + 1]] for i in range(len(new_client_list) - 1))



    return (new_assigned_ordered_matrix, new_transp_demand_matrix, new_route_dist_matrix)
...
def define_available_days(n_days, vehicle_capacity, data, vehicle_f, initial_comb, transp_demand_matrix):

    available_days = np.zeros(n_days, dtype=int)
    # available_days = arr.array([0]*n_days)
    # available_days.count
    for day in range(n_days):
        if (transp_demand_matrix[vehicle_f, day] + data[initial_comb.client, conf.DEMAND_INDEX]) <= vehicle_capacity:
