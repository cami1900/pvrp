...
# FILES
# import classes
from classes import(Move_combination, SwapCombination, Solution_multiOBJ)
import algorithms
...
# Solution_multiOBJ
#(OBJ_value, dist_value, dissim_value, assigned_ordered_matrix, not_assigned_list, transp_demand_matrix, route_dist_matrix, sim_matrix)
...
def AVND_multiOBJ_algorithm(
    ...
    *score_neigh
    ...
    while (current_time - start_time_VND) < time_limit_VND:
        ...
        while iteration < max_iteration_neigh:
            ...
            (initial_comb, final_comb, new_solution, error_index) = \
                define_neighboring_solution(
                    sim_type, weights, neighbour, 
                    n_vehicles, n_days, vehicle_capacity, data, sorted_data, distance_matrix, closeness_matrix, 
                    initial_solution, current_solution, neigh_order, clients_NO_max_frequ) # ho rimosso clients_NO_max_frequ
            ...
            if new_solution.feasibility_vector.verify() == True:
                ...
                # if NS in better than CS (1):
                    ...
                    OBJ_limit_value = (current_solution.OBJ_value * (1 + worse_sol_percentage))
        ...
        # print("*****************************************************************************")
        # print("best_solution.OBJ_value", best_solution.OBJ_value)
        # print("best_solution.OBJ_value", type(best_solution.OBJ_value))
        # print("best_solution.tot_dist", best_solution.tot_dist)
        # print("best_solution.sim_value", best_solution.sim_value)
        # print("\ncurrent_solution.OBJ_value", current_solution.OBJ_value)
        # print("current_solution.tot_dist", current_solution.tot_dist)
        # print("current_solution.sim_value", current_solution.sim_value)
        # print("*****************************************************************************")
...
def MVND_algorithm(worse_sol_acc_VND, neigh_order, max_neighbour, max_iteration_neigh, two_opt_iteration, worse_sol_percentage,
    ...
    for repetition in range(len(worse_sol_acc_VND)):
        ...
        all_best_solutions_VND[repetition+1] = new_solution.OBJ_value
        if new_solution.OBJ_value < best_solution_OBJ:
            best_solution = copy.deepcopy(new_solution)
            best_solution_OBJ = new_solution.OBJ_value
...
def define_neighboring_solution(
        sim_type, weights, neighbour, 
        n_vehicles, n_days, vehicle_capacity, data, sorted_data, distance_matrix, closeness_matrix, 
        initial_solution, current_solution, neigh_order, clients_NO_max_frequ):
    ...
    (final_comb_main, error_index) = def_final_comb(neighbour, n_vehicles, n_days, neigh_order, initial_comb_main)
    if error_index == False:
        final_comb_main = 0
        new_solution = 0
        return (initial_comb_main, final_comb_main, new_solution, False)
    ...
    new_solution = complete_new_solution(
        sim_type=sim_type, 
        weights=weights, 
        ...
        linked_combinations=linked_combinations, 
        initial_comb=initial_comb_main)
...
def def_initial_comb(neighbour, n_vehicles, n_days, data, sorted_data, current_solution, neigh_order, clients_NO_max_frequ):
    ...
    if neigh_order[neighbour] == "k_move_t":
        ...
        initial_comb_main = Move_combination('initial', vehicle_i, day_i, schedule_i, real_client_index)
        # return initial_comb
    #
    # t_move_k
    ...
    if neigh_order[neighbour] == "move_kt":
        ...
        # return initial_comb
    #
    # move_t (maybe also different k)
    if neigh_order[neighbour] == "move_t":
        (real_client_index, error_index) = pick_client(current_solution, vehicle_i, day_i, 0, 0)
    ...
    if neigh_order[neighbour] == "k_swap_t":
        ...
        initial_comb_2 = Move_combination('initial_2', vehicle_i, day_i2, schedule_i2, real_client_index_2)
    ...
    if neigh_order[neighbour] == "t_swap_k":
        ...
        initial_comb_2 = Move_combination('initial_2', vehicle_i2, day_i, schedule_i2, real_client_index_2)
    ...
    if neigh_order[neighbour] == "swap_kt":
        ...
        # return initial_comb
    #
    # swap_t (maybe also different k)
    if neigh_order[neighbour] == "swap_t":
        # Initial combination 1:
        (real_client_index, error_index) = pick_client(current_solution, vehicle_i, day_i, 0, 0) # we are allowed to pick clients with freq = n_days
    ...
    if neigh_order[neighbour] == "t_swap_r_k":
        ...
        # Initial combination 1:
        real_clients_indices = copy.deepcopy(current_solution.assigned_ordered_matrix[vehicle_i, day_i])

        ## si potrebbe togliere!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        schedule_i = np.zeros(n_days)
        schedule_i[day_i] = 1

        initial_comb_1 = Move_combination('initial', vehicle_i, day_i, schedule_i, real_clients_indices)
        ...
        real_clients_indices_2 = copy.deepcopy(current_solution.assigned_ordered_matrix[vehicle_i2, day_i])

        initial_comb_2 = Move_combination('initial_2', vehicle_i2, day_i, schedule_i, real_clients_indices_2)
    ...
    if neigh_order[neighbour] == "t_multiswap_k":
        ...
        client_list_1 = current_solution.assigned_ordered_matrix[vehicle_i, day_i]
        start_idx = np.where(client_list_1 == real_client_index_1[0])[0][0]
        end_idx   = np.where(client_list_1 == real_client_index_1[1])[0][0]
        if start_idx > end_idx:
            start_idx, end_idx = end_idx, start_idx
        set_cliens_index_1 = client_list_1[start_idx : end_idx + 1]

        initial_comb_1 = Move_combination('initial', vehicle_i, day_i, schedule_i, set_cliens_index_1)
        ...
        client_list_2 = current_solution.assigned_ordered_matrix[vehicle_i2, day_i]
        start_idx = np.where(client_list_2 == real_client_index_2[0])[0][0]
        end_idx   = np.where(client_list_2 == real_client_index_2[1])[0][0]
        if start_idx > end_idx:
            start_idx, end_idx = end_idx, start_idx
        set_cliens_index_2 = client_list_2[start_idx : end_idx + 1]

        initial_comb_2 = Move_combination('initial_2', vehicle_i2, day_i, schedule_i, set_cliens_index_2)
...
def def_final_comb(neighbour, n_vehicles, n_days, neigh_order, initial_comb):
    ...
    if neigh_order[neighbour] == "k_move_t":
        ...
        # Final combination:
        final_comb = Move_combination('final', initial_comb.vehicle, day_f, schedule_f, initial_comb.client)
        # return final_comb
    #
    # t_move_k
    if neigh_order[neighbour] == "t_move_k":
        ...
        # Initial combination:
        final_comb = Move_combination('final', vehicle_f, initial_comb.day, schedule_f, initial_comb.client)
        # return final_comb
    #
    # move_kt
    if neigh_order[neighbour] == "move_kt":
        ...
        final_comb = Move_combination('final', vehicle_f, day_f, schedule_f, initial_comb.client)
    #
    # move_t
    if neigh_order[neighbour] == "move_t":
        # pick a random vehicle and a random day 
        day_f = random.choice([t for t in range(0, n_days) if t != initial_comb.day])
        vehicle_f = random.randint(0, n_vehicles-1)
    ...
    if neigh_order[neighbour] == "k_swap_t":
        ...
        # Final combination:
        final_comb_1 = Move_combination('final_1', initial_comb.comb_1.vehicle, initial_comb.comb_2.day, initial_comb.comb_2.schedule, initial_comb.comb_1.client)
        final_comb_2 = Move_combination('final_2', initial_comb.comb_2.vehicle, initial_comb.comb_1.day, initial_comb.comb_1.schedule, initial_comb.comb_2.client)
    ...
    if neigh_order[neighbour] == "t_swap_k":
        # Final combination:
        final_comb_1 = Move_combination('final_1', initial_comb.comb_2.vehicle, initial_comb.comb_1.day, initial_comb.comb_1.schedule, initial_comb.comb_1.client)
        final_comb_2 = Move_combination('final_2', initial_comb.comb_1.vehicle, initial_comb.comb_2.day, initial_comb.comb_2.schedule, initial_comb.comb_2.client)
        final_comb = SwapCombination('final', final_comb_1, final_comb_2)
        # return final_comb
    #
    # f_swap_kt
...
def check_capacity_op(vehicle_capacity, data, neighbour, neigh_order, current_solution, initial_comb_main, final_comb_main):
    ...
    #
    # t_multiswap_k
    elif neigh_order[neighbour] == "t_multiswap_k":

        # comb_1
        start_capacity_1 = current_solution.transp_demand_matrix[final_comb_main.comb_1.vehicle, final_comb_main.comb_1.day]
        demand_clients_1 = 0
        for cl in initial_comb_main.comb_1.client:
            demand_clients_1 += data[cl, conf.DEMAND_INDEX]

        # comb_2   
        start_capacity_2 = current_solution.transp_demand_matrix[final_comb_main.comb_2.vehicle, final_comb_main.comb_2.day]
        demand_clients_2 = 0
        for cl in initial_comb_main.comb_2.client:
            demand_clients_2 += data[cl, conf.DEMAND_INDEX]

        new_capacity_1 = start_capacity_1 - demand_clients_2 + demand_clients_1
        new_capacity_2 = start_capacity_2 - demand_clients_1 + demand_clients_2
...
def check_schedule(n_days, n_vehicles, data, neighbour, neigh_order, current_solution, initial_comb_main, final_comb_main):
    ...
    # NO schedule breakage
          ...
          neigh_order[neighbour] == "t_swap_k" or 
          neigh_order[neighbour] == "t_swap_r_k" or
...
def do_operation(sim_type, weights, neighbour, neigh_order, n_vehicles, n_days, data, distance_matrix, closeness_matrix, 
    ...
    # t_swap_r_k
        ...
        # New solution:
        new_solution = t_swap_r_k(
            sim_type, weights, 
            n_days, n_vehicles, 
            initial_solution, current_solution, initial_comb, final_comb)
   
    # f_swap_kt and f_swap_t
    elif (neigh_order[neighbour] == "f_swap_kt" or 
          neigh_order[neighbour] == "f_swap_t"):
    ...
    # t_multiswap_k
        ...
        # New solution:
        new_solution = muliswap_operation(
            sim_type, weights, 
            n_vehicles, n_days, data, distance_matrix, closeness_matrix, 
            initial_solution, current_solution, initial_comb, final_comb)
...
def move_operation(
    ...
    new_tot_dist = calculate_tot_dist (n_vehicles, n_days, new_route_dist_matrix)

    ''' Update similarity matrix and calculate similarity value '''
    # if sim_type == "sim_1":
    #     new_sim_matrix = sim_calc.update_sim_1_matrix(
    #         n_days, new_assigned_ordered_matrix, new_sim_matrix, 
    #         initial_comb.vehicle)
    #     new_sim_matrix = sim_calc.update_sim_1_matrix(
    #         n_days, new_assigned_ordered_matrix, new_sim_matrix, 
    #         final_comb.vehicle)
    #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
    #         sim_calc.measure_sim_1(n_vehicles, current_solution.sim_matrix, new_sim_matrix)
    
    # elif sim_type == "sim_2":
    #     new_sim_matrix = sim_calc.update_sim_2_matrix(
    ...
    #         final_comb.vehicle, final_comb.day)
    #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
    #         sim_calc.measure_sim_2(n_vehicles, n_days, current_solution.sim_matrix, new_sim_matrix)
    
    # elif sim_type == "sim_3":
    #     new_sim_matrix = sim_calc.update_sim_3_matrix(
    #         new_assigned_ordered_matrix, new_sim_matrix, 
    #         initial_comb.vehicle, initial_comb.day)
    #     new_sim_matrix = sim_calc.update_sim_3_matrix(
    #         new_assigned_ordered_matrix, new_sim_matrix, 
    #         final_comb.vehicle, final_comb.day)
    #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
    #         sim_calc.measure_sim_3(n_vehicles, n_days, current_solution.sim_matrix, new_sim_matrix)
    ...
    if sim_type in ["sim_1", "sim_2", "sim_3"]:
        ...
        new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures = \
            measure_func(n_vehicles, n_days, initial_solution.sim_matrix, new_sim_matrix)
    ...
    # print(f"current_dist = {current_solution.tot_dist}, new_dist = {new_tot_dist} --> dist_value = {new_dist_value}")
    # print(f"sim_value = {new_sim_value} --> dissim_value = {new_dissim_value}")
    # print(f"weights = [{weights[0]}, {weights[1]}] --> OBJ_value = {new_OBJ_value}")

    # Save new solution:
    new_solution = Solution_multiOBJ(
        float(new_OBJ_value), new_tot_dist, float(new_sim_value), 
        new_assigned_ordered_matrix, new_not_assigned_list, new_transp_demand_matrix, new_route_dist_matrix, 
        new_sim_matrix)
...
def swap_operation(
    ...
    '''Remove and add client to the Assigned customer matrix and the Transported demand matrix'''
    (new_assigned_ordered_matrix, new_transp_demand_matrix) = \
        remove_client(data,new_assigned_ordered_matrix, new_transp_demand_matrix, 
    ...
    (new_assigned_ordered_matrix, new_transp_demand_matrix) = \
        add_client(data, new_assigned_ordered_matrix, new_transp_demand_matrix, 
    ...
    (new_route_dist_matrix, new_assigned_ordered_matrix) = \
        optimize_single_route(
            n_vehicles, n_days, distance_matrix, closeness_matrix, 
            new_route_dist_matrix, new_assigned_ordered_matrix, 
            final_comb.comb_1.vehicle, final_comb.comb_1.day)
    ...
    (new_route_dist_matrix, new_assigned_ordered_matrix) = \
        optimize_single_route(
            n_vehicles, n_days, distance_matrix, closeness_matrix, 
            new_route_dist_matrix, new_assigned_ordered_matrix, 
            final_comb.comb_2.vehicle, final_comb.comb_2.day)
    ...
    #     new_sim_matrix = sim_calc.update_sim_1_matrix(
    #         n_days, new_assigned_ordered_matrix, new_sim_matrix, 
    #         final_comb.comb_1.vehicle)
    #     new_sim_matrix = sim_calc.update_sim_1_matrix(
    #         n_days, new_assigned_ordered_matrix, new_sim_matrix, 
    #         final_comb.comb_2.vehicle)
    #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
    #         sim_calc.measure_sim_1(n_vehicles, current_solution.sim_matrix, new_sim_matrix)
...
def muliswap_operation(
    ...
    '''Remove and add client to the Assigned customer matrix and the Transported demand matrix'''
    for cl_1 in initial_comb.comb_1.client:
        (new_assigned_ordered_matrix, new_transp_demand_matrix) = \
            remove_client(data,new_assigned_ordered_matrix, new_transp_demand_matrix, 
                            initial_comb.comb_1.day, initial_comb.comb_1.vehicle, cl_1)
        (new_assigned_ordered_matrix, new_transp_demand_matrix) = \
            add_client(data, new_assigned_ordered_matrix, new_transp_demand_matrix, 
                        final_comb.comb_1.day, final_comb.comb_1.vehicle, cl_1)
    for cl_2 in initial_comb.comb_2.client:
        (new_assigned_ordered_matrix, new_transp_demand_matrix) = \
            remove_client(data,new_assigned_ordered_matrix, new_transp_demand_matrix, 
                            initial_comb.comb_2.day, initial_comb.comb_2.vehicle, cl_2)
        (new_assigned_ordered_matrix, new_transp_demand_matrix) = \
            add_client(data, new_assigned_ordered_matrix, new_transp_demand_matrix, 
                        final_comb.comb_2.day, final_comb.comb_2.vehicle, cl_2)
    ...
    # print(f"\n\n*******************")
    # print(f"current_solution.tot_dist: {current_solution.tot_dist}")
    # print("new_tot_dist:", new_tot_dist)
    # print("new_dist_value:", new_dist_value)
    # print("new_sim_value:", new_sim_value)
    # print("new_dissim_value:", new_dissim_value)
    # print("new_OBJ_value:", new_OBJ_value)
    # print("*******************\n\n")
...
def t_swap_r_k(sim_type, weights, n_days, n_vehicles, initial_solution, current_solution, initial_comb, final_comb):

    vehicle_i = initial_comb.comb_1.vehicle
    day_i = initial_comb.comb_1.day
    clients_set_i_i = initial_comb.comb_1.client

    vehicle_f = initial_comb.comb_2.vehicle
    clients_set_f_i = initial_comb.comb_2.client

    ''' swap '''
    assigned_clients_new = copy.deepcopy(current_solution.assigned_ordered_matrix)
    
    assigned_clients_new[vehicle_i, day_i] = clients_set_f_i
    assigned_clients_new[vehicle_f, day_i] = clients_set_i_i

    ''' similarity values '''
    # min(max) --> try to minimize the vehicle with higher number of clients
    sim_int_value_current = sim_calc.measure_sim_int(n_vehicles, n_days, current_solution.assigned_ordered_matrix)
    sim_int_value_new = sim_calc.measure_sim_int(n_vehicles, n_days, assigned_clients_new)
...
#    ''' MOVE '''
    if (neigh_order[neighbour] == "k_move_t" or 
        ...
        for comb_vehicle, comb_day in linked_combinations:
            ...
            #     continue
            (new_assigned_ordered_matrix, new_transp_demand_matrix) = remove_client(data, new_assigned_ordered_matrix, new_transp_demand_matrix, 
...
def complete_new_solution(sim_type, weights, neigh_order, neighbour, 
                          data, n_vehicles, n_days, distance_matrix, closeness_matrix, 
                          new_solution, initial_solution, current_solution, selected_combinations, linked_combinations, initial_comb):
    ...
    if (neigh_order[neighbour] == "k_move_t" or 
        ...
        # if sim_type == "sim_1":
        #     for comb_vehicle, comb_day in selected_combinations:
        #         new_solution.sim_matrix = sim_calc.update_sim_1_matrix(n_days, 
        #             new_solution.assigned_ordered_matrix, new_solution.sim_matrix, 
        #             comb_vehicle)
        #     for comb_vehicle, comb_day in linked_combinations:
        #         new_solution.sim_matrix = sim_calc.update_sim_1_matrix(n_days, 
        #             new_solution.assigned_ordered_matrix, new_solution.sim_matrix, 
        #             comb_vehicle)
        #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
        #         sim_calc.measure_sim_1(n_vehicles, current_solution.sim_matrix, new_solution.sim_matrix)
        
        # elif sim_type == "sim_2":
        #     for comb_vehicle, comb_day in selected_combinations:
        #         new_solution.sim_matrix = sim_calc.update_sim_2_matrix(
        #             new_solution.assigned_ordered_matrix, new_solution.sim_matrix, 
        #             comb_vehicle, comb_day)
        #     for comb_vehicle, comb_day in linked_combinations:
        #         new_solution.sim_matrix = sim_calc.update_sim_2_matrix(
        ...
        #             comb_vehicle, comb_day)
        #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
        #         sim_calc.measure_sim_2(n_vehicles, n_days, current_solution.sim_matrix, new_solution.sim_matrix)
        
        # elif sim_type == "sim_3":
        #     for comb_vehicle, comb_day in selected_combinations:
        #         new_solution.sim_matrix = sim_calc.update_sim_3_matrix(
        #             new_solution.assigned_ordered_matrix, new_solution.sim_matrix, 
        #             comb_vehicle, comb_day)
        #     for comb_vehicle, comb_day in linked_combinations:
        #         new_solution.sim_matrix = sim_calc.update_sim_3_matrix(
        ...
        #             comb_vehicle, comb_day)
        #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
        #         sim_calc.measure_sim_3(n_vehicles, n_days, current_solution.sim_matrix, new_solution.sim_matrix)
        
        if sim_type in ["sim_1", "sim_2", "sim_3"]:
            _, update_func, measure_func = get_sim_functions(sim_type, sim_calc)

            for combination_group in [selected_combinations, linked_combinations]:
                for comb_vehicle, comb_day in combination_group:            ### ---------->>> POSSIBLE ISSUE
                    new_solution.sim_matrix = update_func(
                        n_days,
                        new_solution.assigned_ordered_matrix,
                        new_solution.sim_matrix,
    ...
    #    ''' SWAP 1x1 '''
        ...
        # if sim_type == "sim_1":
        #     for combination_group in [selected_combinations, linked_combinations]:
        #         for sublist in combination_group:
        #             for comb_vehicle, comb_day in sublist:
        #                 new_solution.sim_matrix = sim_calc.update_sim_1_matrix(n_days,
        #                     new_solution.assigned_ordered_matrix, new_solution.sim_matrix,
        #                     comb_vehicle, comb_day)
        ...
        #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
        #     sim_calc.measure_sim_1(n_vehicles, n_days, current_solution.sim_matrix, new_solution.sim_matrix)

        # if sim_type == "sim_2":
        #     for combination_group in [selected_combinations, linked_combinations]:
        #         for sublist in combination_group:
        #             for comb_vehicle, comb_day in sublist:
        #                 new_solution.sim_matrix = sim_calc.update_sim_2_matrix(n_days,
        #                     new_solution.assigned_ordered_matrix, new_solution.sim_matrix, 
        #                     comb_vehicle, comb_day)
        #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
        #         sim_calc.measure_sim_2(n_vehicles, n_days, current_solution.sim_matrix, new_solution.sim_matrix)
        ...
        #         for sublist in combination_group:
        #             for comb_vehicle, comb_day in sublist:
        #                 new_solution.sim_matrix = sim_calc.update_sim_3_matrix(n_days,
        #                     new_solution.assigned_ordered_matrix, new_solution.sim_matrix, 
        #                     comb_vehicle, comb_day)
        #     (new_sim_value, sim_combinations_matrix, sim_vehicle_pairings, sim_measures) = \
        #         sim_calc.measure_sim_3(n_vehicles, n_days, current_solution.sim_matrix, new_solution.sim_matrix)
    ...
    # print(f"current_dist = {current_solution.tot_dist}, new_dist = {new_tot_dist} --> dist_value = {new_dist_value}")
    # print(f"sim_value = {new_sim_value} --> dissim_value = {new_dissim_value}")
    # print(f"weights = [{weights[0]}, {weights[1]}] --> OBJ_value = {new_solution.OBJ_value}")
...
def crete_new_sol_attributes(current_solution):
    ...
    new_sim_matrix = copy.deepcopy(current_solution.sim_matrix)

    return (new_OBJ_value, new_tot_dist, new_sim_value, new_assigned_ordered_matrix, new_not_assigned_list, new_transp_demand_matrix, new_route_dist_matrix, new_sim_matrix)


def remove_client(data, new_assigned_ordered_matrix, new_transp_demand_matrix, initial_day, initial_vehicle, client):
...
def add_client(data, new_assigned_ordered_matrix, new_transp_demand_matrix, final_day, final_vehicle, client):
...
def calculate_tot_dist (n_vehicles, n_days, route_dist_matrix):

    tot_dist = 0
    for vehicle_k in range(n_vehicles):
        for day_t in range(n_days):
            tot_dist += route_dist_matrix[vehicle_k][day_t]
