...
class TSP(object):
    ...
    def perform_2_opt_swap(self):
        ...
        return cities
    # do NOT move depot:
    def perform_2_opt_swap_NO_depot(self):
        """
        Performs 2-opt swap 5 times, keeping the depot (first and last node) fixed.
        """
        cities = self.random_solution.copy()
        size_of_cities = len(cities)

        self.logger.info('-------Running 2 Opt Perturbation 5 times (depot fixed)-------')

        for _ in range(5):
            # scegli indici solo tra i clienti, non il depot
            c1 = random.randrange(1, size_of_cities - 2)
            c2 = random.randrange(1, size_of_cities - 2)

            if c2 < c1:
                c1, c2 = c2, c1

            # inverte il segmento interno
            cities[c1:c2] = reversed(cities[c1:c2])
